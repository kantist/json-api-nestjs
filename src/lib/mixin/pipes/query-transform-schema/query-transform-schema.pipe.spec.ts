...
describe('QueryTransformSchema', () => {
	...
	const querySchemaMixinPip = queryTransformSchemaMixin(
		Users,
		mockConnectionName
	);
		...
		it('Error in field object', async () => {
			...
			try {
				...
				const countError = e.response.message.filter(
					(item) => item.source.parameter.split('/')[1] === 'fields'
				).length;
	...
	describe('Check include', () => {
		it('Should be incorrect with empty array', async () => {
			...
			try {
				...
				const countError = e.response.message.filter(
					(item) => item.source.parameter.split('/')[1] === 'include'
				).length;
				expect(e.response.message.length).toBe(countError);
				expect(e.response.message.length).toBeGreaterThan(0);
				expect(e).toBeInstanceOf(BadRequestException);
	...
	describe('Check sort', () => {
		it('Should be incorrect', async () => {
			...
			try {
				...
				const countError = e.response.message.filter(
					(item) => item.source.parameter.split('/')[1] === 'sort'
				).length;
				expect(e.response.message.length).toBeGreaterThan(0);
				expect(e.response.message.length).toBe(countError);
				expect(e).toBeInstanceOf(BadRequestException);
