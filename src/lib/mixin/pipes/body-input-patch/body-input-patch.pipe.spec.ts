...
describe('BodyInputPatchPipe', () => {
	...
	const bodyInputPostMixinPip = bodyInputPatchPipeMixin(
		Users,
		mockConnectionName
	);
	...
	it('Should be error incorrect field', async () => {
		...
		try {
			...
			expect(e.response.message.length).toBeGreaterThan(0);
			expect(e.response.message[0].detail).toBe(
				'Must NOT have additional properties. Additional Property is: "someField"'
			);
			expect(e.response.message[1].detail).toBe(
				'Must NOT have additional properties. Additional Property is: "someRelation"'
			);
		}
	});
});
