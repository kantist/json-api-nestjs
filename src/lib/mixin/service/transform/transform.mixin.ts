...
import { camelToKebab, getEntityName, snakeToCamel } from '../../../helper';
...
export class TransformMixinService<T> {
	...
	private relationPrimaryField: Map<string, string> =
		this.repository.metadata.relations.reduce((acum, item) => {
			const relationMetadata = this.repository.manager.getRepository(
				item.inverseEntityMetadata.target
			).metadata;
			acum.set(
				item.propertyName,
	...
	private relationColumns: Set<string> =
		this.repository.metadata.relations.reduce(
			(a, i) => (a.add(i.propertyName), a),
			new Set<string>()
		);
	...
	private commonColumnsForRelation: Map<string, Set<string>> =
		this.repository.metadata.relations.reduce((acum, item) => {
			const relationMetadata = this.repository.manager.getRepository(
				item.inverseEntityMetadata.target
			).metadata;
			...
			acum.set(item.propertyName, columns);

			return acum;
		}, new Map<string, Set<string>>());

	private relationColumnsForRelation: Map<string, Set<string>> =
		this.repository.metadata.relations.reduce((acum, item) => {
			const relationMetadata = this.repository.manager.getRepository(
				item.inverseEntityMetadata.target
			).metadata;
			const columns = relationMetadata.relations.reduce(
				(a, i) => (a.add(i.propertyName), a),
				new Set<string>()
			);
			acum.set(item.propertyName, columns);
	...
	private relationTarget: Map<string, Entity> =
		this.repository.metadata.relations.reduce((acum, item) => {
			acum[item.propertyName] = item.inverseEntityMetadata.target;
			acum.set(
				item.propertyName,
        item.inverseEntityMetadata.target as Function
			);

			return acum;
		}, new Map<string, Entity>());

	private isArrayRelations: Record<string, boolean> =
	...
	transformRawData(data: any): T[] {
		...
		for (let i = 0; i < countData; i++) {
			...
			for (const field in currentItem) {
				...
				if (
					tableName === this.currentResourceName &&
          this.commonColumns.has(fieldName)
				) {
          ...
          this.commonColumnsForRelation.get(tableName).has(fieldName)
			...
			dataJson[currentId] = {
				...
				...Object.entries(relationObject).reduce((acum, [key, val]) => {
					...
					const plainObject = plainToClass(
            this.relationTarget.get(key) as any,
            val
					);
		...
		return Object.values(dataJson).map((i) =>
			plainToClass(this.repository.target as any, i)
		);
	}
	...
	transformData<T>(
      ...
      table === this.currentResourceName
      	...
      	: this.relationColumnsForRelation.get(table);
	...
	transformInclude<T>(data: T[]): ResourceObject<T>['included'] {
		...
		for (const itemRow of data) {
			const needField = Object.keys(itemRow).filter((i) =>
				this.relationColumns.has(i)
			);
			for (const field of needField) {
				if (
					!itemRow[field] ||
          (Array.isArray(itemRow[field]) && itemRow[field].lenght > 0)
				) {
					continue;
				}

				result[field] = result[field] || {};
				const includeArray = Array.isArray(itemRow[field])
					? itemRow[field]
					: [itemRow[field]];
		...
		return Object.keys(result).reduce<ResourceObject<T>['included']>(
			(acum, item) => {
	...
	private getPrefixArray(): string[] {
		...
		if (version && version.type === VersioningType.URI) {
			...
			if (firstVersion) {
				urlPath.push(
					`${this.routePathFactory.getVersionPrefix(
						version
