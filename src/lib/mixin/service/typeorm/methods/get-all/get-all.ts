...
export async function getAll<T>(
	...
	const resultBuilderQuery =
    this.repository.createQueryBuilder(preparedResourceName);

	if (!this.UtilsMethode.relationNameMapByEntity.has(preparedResourceName)) {
		this.UtilsMethode.relationNameMapByEntity.set(
			preparedResourceName,
			new Set(
				this.repository.metadata.relations.map((item) => item.propertyName)
			)
		);
	}
	...
	if (fields) {
		...
		for (const rel of include || []) {
			const propsName = this.repository.metadata.relations.find(
				(relations) => relations.propertyName === rel
	...
	for (let i = 0; i < includeLength; i++) {
		...
		resultBuilderQuery.leftJoin(
			`${preparedResourceName}.${relationName}`,
			relationName
		);
	}
	...
	if (sort) {
		...
		const relationOrder = Object.keys(otherSort || {}).reduce<OrderByCondition>(
			(acum, relation) => {
				return Object.entries<SortType>(
					(otherSort || {})[relation]
				).reduce<OrderByCondition>((a, [key, val]) => {
					a[`${relation}.${key}`] = val;
