...
describe('PatchRelationship methode test', () => {
	...
	beforeAll(async () => {
		...
		const transformMixinServiceComments = transformMixin(
			Comments,
			DEFAULT_CONNECTION_NAME
		);
		const typeormMixinServiceComments = typeormMixin(
			Comments,
			DEFAULT_CONNECTION_NAME,
      ...
      module.get<TypeormMixinService<Users>>(typeormMixinService);
		typeormServiceComments = module.get<TypeormMixinService<Comments>>(
			typeormMixinServiceComments
		);
		...
		const addresses1 = await repository.manager.getRepository(Addresses).save(
			Object.assign(new Addresses(), {
				country: 'country1',
				state: 'state1',
			})
		...
		const roles2 = await repository.manager.getRepository(Roles).save(
			Object.assign(new Roles(), {
				name: 'user2',
				key: 'USER2',
				isDefault: true,
		...
		await repository.manager.getRepository(Comments).save(
		...
		const manager = await repository.save(
