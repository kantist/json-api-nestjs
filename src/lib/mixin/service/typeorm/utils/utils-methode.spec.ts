...
import { OperandMapForNull, OperandsMap } from '../../../../types-common';
...
describe('Utils methode test', () => {
	...
	beforeAll(async () => {
		...
		const address = await repository.manager.getRepository(Addresses).save(
	...
	describe('UtilsMethode.applyQueryFiltersTarget', () => {
		it('expression for target field', () => {
			...
			for (let i = 0; i < params2.length; i++) {
				...
				expect(expression2[i].expression).toBe(
					`${alias}.${params2[i]} ${checkExpression}`
				);
				if (operand === FilterOperand.like) {
					expect(expression2[i].params.val).toBe(
						`%${filter2[params[i]][operand]}%`
					);
				} else {
			...
			for (let i = 0; i < params3.length; i++) {
				...
				expect(expression3[i].expression).toBe(
					`${alias}.${params3[i]} ${checkExpression}`
				);
	...
	describe('UtilsMethode.applyQueryFilterRelation', () => {
		it('expression for relation many-to-one', async () => {
			...
			for (let i = 0; i < params2.length; i++) {
				...
				if (operand === FilterOperand.like) {
					expect(expression2[i].params.val).toBe(
						`%${filter2.manager[params[i]][operand]}%`
					);
				} else {
					expect(expression2[i].params.val).toBe(
						filter2.manager[params[i]][operand]
			...
			for (let i = 0; i < params3.length; i++) {
				...
				const checkExpression = OperandMapForNull[operand].replace(
					'EXPRESSION',
					paramsName
				);
				expect(expression3[i].expression).toBe(
	...
	describe('UtilsMethode.asyncIterateFindRelationships', () => {
		...
		it('should be error: ids does not exist', async () => {
			...
			try {
				...
				expect(e.response.detail).toBe(
					`Resource 'roles' with ids '${data.roles.data
						.map((i) => i.id)
						.join(',')}' does not exist`
				);
			}
		...
		it('should be error: id does not exist', async () => {
			...
			try {
				...
				expect(e).toBeInstanceOf(NotFoundException);
				expect(e.response.detail).toBe(
					`Resource 'addresses' with id '${data.addresses.data.id}' does not exist`
				);
			}
		});
	...
	describe('UtilsMethode.validateRelationRequestData', () => {
		it('Should be error, should be array', async () => {
			...
			expect.assertions(3);
			try {
				await UtilsMethode.validateRelationRequestData(
					repository,
					1,
					'roles',
					dataBody
		...
		it('Should be error, incorrect type in array', async () => {
			...
			try {
				...
				expect(e.response.message[0].detail).toBe(
					'Type should be equal type of relName: "roles". Type of "roles" is roles'
				);
		...
		it('Should be error, incorrect type in object', async () => {
			...
			try {
				...
				expect(e).toBeInstanceOf(UnprocessableEntityException);
				expect(e.response.message[0].detail).toBe(
					'Type should be equal type of relName: "addresses". Type of "addresses" is addresses'
				);
				expect(e.response.message[0].source.parameter).toBe('/data');
			}
		});
		...
		it('Should be error, rel is not exist', async () => {
			...
			try {
				...
				expect(e).toBeInstanceOf(NotFoundException);
				expect(e.response.message[0].detail).toBe(
					'Not exist item "3" in relation "roles"'
				);
			}
		});
		...
		it('Should be error, main item not exist', async () => {
			...
			try {
				...
				expect(e).toBeInstanceOf(NotFoundException);
				expect(e.response.message[0].detail).toBe(
					'Not exist item "5" in resource "users"'
				);
			}
		});
		...
		it('Should be ok', async () => {
			const dataBody = [{ type: 'roles', id: '1' }] as any;
			const itemId = 1;
			const { item, rel } = await UtilsMethode.validateRelationRequestData(
				repository,
				itemId,
				'roles',
				dataBody
