...
export declare type SortRules<T> = {
...
export declare type Includes<T> = EntityRelation<T>[];
...
declare type MapFilterOperandTypeString = Exclude<FilterOperand, FilterOperand.in | FilterOperand.nin | FilterOperand.some>;
export declare type Operands = {
    [P in FilterOperand]: Record<P, P extends MapFilterOperandTypeString ? string : string[]> & Partial<Record<Exclude<FilterOperand, P>, never>> extends infer O ? {
        [Q in keyof O]: O[Q];
    } : never;
}[FilterOperand];
export declare type OperandsRelation = {
    [P in Extract<FilterOperand, FilterOperand.eq | FilterOperand.ne>]: Record<P, string> & Partial<Record<Exclude<Extract<FilterOperand, FilterOperand.eq | FilterOperand.ne>, P>, never>> extends infer O ? {
        [Q in keyof O]: O[Q];
    } : never;
...
export declare type QuerySchemaTypes = {
    [QueryField.page]?: {
        number?: string;
        size?: string;
    };
